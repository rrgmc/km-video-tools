...
This program is a single file somebody copies onto a desktop; a `static/` directory that had to
arrive beside it is 55 KB of ways to be half-installed.
...
- **A listener on `document.body` does not survive a swap of the body.** Setting the output folder
  ...
  handling above works exactly until somebody chooses a folder, after which every refusal is silent
  — a perfectly good "that is not a browser yt-dlp knows" goes nowhere. htmx's events bubble to
  `document`, which nothing replaces, so that is where they are bound.
